...
export const getWrapperComponent = (
  ...
  class WrapperComponent extends Component {
      ...
      if (storeStruct && storeStruct.length > 0) {
        ...
        if (missingDataList.length > 0) {
          logger.error(
            ...
            } - Component failed to recieve critical data`,
    ...
    static async checkUserState({ isServer, req, requestDetails, store }) {
      let email;
    ...
    static async getInitialProps(...params) {
      ...
      if (isServer) {
        ...
        store.dispatch(
          serverActions.addActiveBrand(
            process.env.ACTIVE_BRAND || DEFAULT_BRAND
          )
        );
        store.dispatch(serverActions.setPageUrl(req.url));
        store.dispatch(serverActions.setPageQuery({ ...req.query, ...query }));
        store.dispatch(
          serverActions.setPageOrigin(`${req.protocol}://${req.headers.host}`)
        );

        if (req.cookies && req.cookies.pdpCrumb) {
          store.dispatch(
            serverActions.setPDPCrumb(JSON.parse(req.cookies.pdpCrumb))
          );
        }
        ...
        store.dispatch({
          configKeys: fromJS(cache.get('configKeys')),
          type: CONFIG_KEYS
        });
      } else {
        ...
        requestDetails = {
          deviceType: store
            .getState()
            .getIn(['global', 'globalData', 'deviceType'])
        };
      }
